...
# Plugin files
if [ -f "$DATA_DIR/plugins/hermes-finance-bi-plugin/plugin.yaml" ]; then
  pass "plugin.yaml present"
else
  fail "plugin missing under plugins/hermes-finance-bi-plugin"
fi
...
do
  if grep -q "$tool" "$DATA_DIR/plugins/hermes-finance-bi-plugin/plugin.yaml" 2>/dev/null; then
...
if [ -f "$INSTANCE_ENV" ]; then
  ...
  if [ -z "$DSN_VAL" ]; then
    warn "FINANCE_BI_DSN is empty (configure readonly DSN before production use)"
  fi
else
...
export PYTHONPATH="$DATA_DIR/plugins/hermes-finance-bi-plugin${PYTHONPATH:+:$PYTHONPATH}"
...
if docker inspect "$CONTAINER" >/dev/null 2>&1; then
  if docker exec "$CONTAINER" bash -lc 'hermes plugins list 2>/dev/null | grep -qi finance || hermes plugins list 2>/dev/null | grep -qi bi || true' >/tmp/hermes-bi-plugins.out 2>&1; then
    if grep -qiE 'finance|bi' /tmp/hermes-bi-plugins.out 2>/dev/null; then
      pass "hermes plugins list mentions finance/bi"
    else
      warn "container up but plugins list did not clearly show finance-bi (check Hermes plugin discovery)"
    fi
  else
    warn "could not run hermes plugins list in container"
  fi
